...
local function onMode(x, y)
    ...
    for name, M in next, MODES do
        if RANKS[name] and M.x then
            local s = M.size
            if M.shape == 1 then
                if x > M.x - s and x < M.x + s and y > M.y - s and y < M.y + s then
                    return name
                end
            elseif M.shape == 2 then
                if abs(x - M.x) + abs(y - M.y) < s then
                    return name
                end
            elseif M.shape == 3 then
                if (x - M.x) ^ 2 + (y - M.y) ^ 2 < s ^ 2 then
                    return name
                end
            end
        end
...
local function moveMap(dx, dy)
    local k = getK()
    local x, y = getPos()
    if x > 1300 and dx < 0 or x < -1200 and dx > 0 then
        dx = 0
    end
    if y > 420 and dy < 0 or y < -1900 and dy > 0 then
        dy = 0
    end
...
function Tmr.mode()
    ...
    if not SCN.swapping then
        if kb.isDown("up", "w") then
            dy = dy + 10
            F = true
        end
        if kb.isDown("down", "s") then
            dy = dy - 10
            F = true
        end
        ...
        if js1 then
            ...
            if dir ~= "c" then
                if dir == "u" or dir == "ul" or dir == "ur" then
                    dy = dy + 10
                    F = true
                end
                if dir == "d" or dir == "dl" or dir == "dl" then
                    dy = dy - 10
                    F = true
                end
                if dir == "l" or dir == "ul" or dir == "dl" then
                    dx = dx + 10
                    F = true
                end
                if dir == "r" or dir == "ur" or dir == "dr" then
                    dx = dx - 10
                    F = true
                end
            end
...
function Pnt.mode()
    ...
    if sel then
        ...
        gc.setColor(M.color)
        setFont(40)
        mStr(text.modes[sel][1], 1100, 5)
        setFont(30)
        mStr(text.modes[sel][2], 1100, 50)
        gc.setColor(1, 1, 1)
        setFont(25)
        gc.printf(text.modes[sel][3], 920, 110, 360, "center")
        if M.slowMark then
...
WIDGET.init("mode", {
    WIDGET.newButton({ name = "start", x = 1040, y = 655, w = 180, h = 80, font = 40, code = WIDGET.lnk_pressKey("return"), hide = function()
        return not mapCam.sel
    end }),
