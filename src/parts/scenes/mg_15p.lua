    ...
    if S.state < 2 then
        ...
        if moves > 0 then
            ...
            if checkBoard(b) then
                ...
                if S.time < 1 then
                    LOG.print("不是人", COLOR.lBlue)
                elseif S.time < 2 then
                    LOG.print("还是人", COLOR.lBlue)
                elseif S.time < 3 then
                    LOG.print("神仙", COLOR.lBlue)
                elseif S.time < 5 then
                    LOG.print("太强了", COLOR.lBlue)
                elseif S.time < 7.5 then
                    LOG.print("很强", COLOR.lBlue)
                elseif S.time < 10 then
                    LOG.print("可以的", COLOR.lBlue)
                elseif S.time < 20 then
                    LOG.print("马上入门了", COLOR.lBlue)
                elseif S.time < 30 then
                    LOG.print("入门不远了", COLOR.lBlue)
                elseif S.time < 60 then
                    LOG.print("多加练习", COLOR.lBlue)
                else
                    LOG.print("第一次玩?加油", COLOR.lBlue)
                end
                SFX.play("win")
            end
            SFX.play("move")
...
WIDGET.init("mg_15p", {
    ...
    WIDGET.newSlider({ name = "color", x = 110, y = 250, w = 170, unit = 4, show = false, font = 30, disp = WIDGET.lnk_STPval("color"), code = function(v)
        if sceneTemp.state ~= 1 then
            sceneTemp.color = v
        end
    end, hide = WIDGET.lnk_STPeq("state", 1) }),
    ...
    WIDGET.newSwitch({ name = "pathVis", x = 240, y = 510, w = 60, font = 40, disp = WIDGET.lnk_STPval("pathVis"), code = WIDGET.lnk_pressKey("r"), hide = function()
        return sceneTemp.state == 1 or not sceneTemp.slide
    end }),
