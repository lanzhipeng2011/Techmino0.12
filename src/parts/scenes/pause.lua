...
function sceneInit.pause(org)
    ...
    if f == 1 then
        S.color, f = { .4, .9, .5 }, 1.25    --Vegetable
    elseif f == 2 then
        S.color, f = { .4, .7, .9 }, 1        --Normal
    elseif f == 3 then
        S.color, f = { 1, .3, .3 }, .626    --Diao
    end
    A = {
        120 * .5 * f, 120 * 3 ^ .5 * .5 * f,
        120 * -.5 * f, 120 * 3 ^ .5 * .5 * f,
...
function Pnt.pause()
    ...
    if T > .5 and GAME.frame > 180 then
        ...
        gc.push("transform")
        gc.scale((3 - 2 * T) * T)
        gc.setColor(1, 1, 1, T * (.5 + .3 * sin(Timer() * 6.26)))
...
WIDGET.init("pause", {
    ...
    WIDGET.newButton({ name = "replay", x = 640, y = 250, w = 240, h = 100, color = "lYellow", font = 30, code = WIDGET.lnk_pressKey("p"), hide = function()
        return not (GAME.result or GAME.replaying) or #PLAYERS > 1
    end }),
