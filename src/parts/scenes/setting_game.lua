...
WIDGET.init("setting_game", {
    ...
    WIDGET.newSlider({ name = "reTime", x = 350, y = 340, w = 300, unit = 10, font = 30, disp = WIDGET.lnk_SETval("reTime"), code = WIDGET.lnk_SETsto("reTime"), show = function(S)
        return (.5 + S.disp() * .25) .. "s"
    end }),
    ...
    WIDGET.newSwitch({ name = "fine", x = 1080, y = 440, font = 20, disp = WIDGET.lnk_SETval("fine"), code = function()
        SETTING.fine = not SETTING.fine
        if SETTING.fine then
            SFX.play("finesseError", .6)
        end
    end }),
    WIDGET.newSwitch({ name = "appLock", x = 1080, y = 500, font = 20, disp = WIDGET.lnk_SETval("appLock"), code = WIDGET.lnk_SETrev("appLock") }),
    WIDGET.newButton({ name = "calc", x = 970, y = 550, w = 150, h = 60, color = "dGrey", font = 25, code = WIDGET.lnk_goScene("calculator"), hide = function()
        return not SETTING.appLock
    end }),
