...
local function updateLine(P)
    --Attacks, line pushing, cam moving
...
function update.alive(P, dt)
    ...
    if P.keyRec then
        ...
        local v = 0
        for i = 2, 10 do
            v = v + i * (i - 1) * 7.2 / (_ - P.keyTime[i] + 1)
        end
        P.keySpeed = P.keySpeed * .99 + v * .1

        v = 0
        for i = 2, 10 do
            v = v + i * (i - 1) * 7.2 / (_ - P.dropTime[i])
        end
    ...
    if not P.keepVisible then
        for j = 1, #P.field do
            for i = 1, 10 do
                if P.visTime[j][i] > 0 then
                    P.visTime[j][i] = P.visTime[j][i] - 1
                end
            end
        end
    end
