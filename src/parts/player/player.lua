...
local kickList = require("src/parts/kickList")
...
function Player.createBeam(P, R, send, color)
    local x1, y1, x2, y2
    if P.small then
        x1, y1 = P.centerX, P.centerY
    else
        x1, y1 = P.x + (30 * (P.curX + P.sc[2]) - 30 + 15 + 150) * P.size, P.y + (600 - 30 * (P.curY + P.sc[1]) + 15 + 70) * P.size
    end
    if R.small then
...
function Player.set20G(P, if20g, init)
    ...
    if init and if20g and P.AI_mode == "CC" then
        CC.switch20G(P)
    end
end
...
function Player.ifoverlap(P, bk, x, y)
    ...
    for i = 1, #bk do
        if P.field[y + i - 1] then
            for j = 1, C do
                if bk[i][j] and P.field[y + i - 1][x + j - 1] > 0 then
                    return true
                end
            end
        end
...
function Player.attack(P, R, send, time, ...)
    ...
    if R.atkBuffer.sum < 20 then
        local B = R.atkBuffer
        if send > 20 - B.sum then
            send = 20 - B.sum
        end--No more then 20
        local m, k = #B, 1
        while k <= m and time > B[k].countdown do
            k = k + 1
        end
        for i = m, k, -1 do
            B[i + 1] = B[i]
...
function Player.lock(P)
    ...
    for i = 1, P.r do
        local y = P.curY + i - 1
        if not P.field[y] then
            P.field[y], P.visTime[y] = FREEROW.get(0), FREEROW.get(0)
        end
        for j = 1, P.c do
            if P.cur.bk[i][j] then
...
function Player.hold(P, ifpre)
    if not P.holded and (ifpre or P.waiting == -1) and P.gameEnv.hold then
        ...
        if P.next[1] or C then
            --Make hold available in fixed sequence
...
function Player.popNext(P)
    --Pop next queue to hand
    P.holded = false
    P.spinLast = false
    P.spinSeq = 0
    P.ctrlCount = 0
...
do
    ...
    for i = 2, 4 do
        finesseList[6][i] = finesseList[6][1]
    end
    ...
    function Player.drop(P)
        ...
        if not finesse then
            ...
            end--Allow 2 more step for roof-less spin
            local id = CB.id
        ...
        piece.special = false
        if cc > 0 then
            --If lines cleared, about 200 lines below
            cmb = cmb + 1
            if dospin then
        ...
        if dospin then
            _ = STAT.spin[n]
            _[cc + 1] = _[cc + 1] + 1--Spin[1~25][0~4]
            _ = STAT.spins
            _[cc + 1] = _[cc + 1] + 1--Spin[0~4]
        elseif cc > 0 then
            _ = STAT.clear[n]
            _[cc] = _[cc] + 1--Clear[1~25][1~5]
            _ = STAT.clears
            _[cc] = _[cc] + 1--Clear[1~5]
        end
...
function Player.act_moveLeft(P, auto)
    ...
    elseif P.control and P.waiting == -1 then
        if P.cur and not P:ifoverlap(P.cur.bk, P.curX - 1, P.curY) then
            ...
            P.curX = P.curX - 1
            P:freshBlock(false, true)
            if P.sound and P.curY == P.imgY then
                SFX.play("move")
            end
            if not auto then
                P.moving = 0
