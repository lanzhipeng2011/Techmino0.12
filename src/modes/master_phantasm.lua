...
local function check_LVup(P)
    ...
    if p >= P.gameEnv.target then
        ...
        if T == 12 then
            ...
            SFX.play("reach")
        elseif T == 26 then
            --Stage 3: dig to bottom
            if not P.hd then
                P.life = P.life + 1
            end--1 up if ban hold
            P.waiting = 45
            ENV.skin[1], ENV.skin[2] = ENV.skin[2], ENV.skin[1]
            ENV.skin[3], ENV.skin[4] = ENV.skin[4], ENV.skin[3]
        ...
        elseif T == 62 then
            --Stage 5: survive without easy-fresh rule
            P.life = 1
            ENV.lock = 13
            ENV.wait = 5
            ENV.fall = 5
...
return {
    ...
    score = function(P)
        return { P.result == "WIN" and 260 or P.modeData.point, P.stat.time }
    end,
    scoreDisp = function(D)
        return D[1] .. "P   " .. toTime(D[2])
    end,
    comp = function(a, b)
