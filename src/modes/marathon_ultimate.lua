...
local function check_LVup(P)
    if P.combo > 1 or P.b2b > 0 or P.lastPiece.row > 1 then
        if P.combo > 1 then
            P:showText("2x", 0, -220, 40, "flicker", .3)
        end
        if P.b2b > 0 then
            P:showText("spin", 0, -160, 40, "flicker", .3)
        end
        if P.lastPiece.row > 1 then
            P:showText("1+", 0, -100, 40, "flicker", .3)
        end
        P:lose()
        return
    end
...
return {
    ...
    score = function(P)
        return { math.min(P.stat.row, 200), P.stat.time }
    end,
    scoreDisp = function(D)
        return D[1] .. " Lines   " .. toTime(D[2])
    end,
    comp = function(a, b)
