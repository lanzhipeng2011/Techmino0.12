local langList = {
    require("resources/languages/lang_zh"),
    require("resources/languages/lang_zh2"),
    require("resources/languages/lang_en"),
    require("resources/languages/lang_fr"),
    require("resources/languages/lang_sp"),
    require("resources/languages/lang_symbol"),
    require("resources/languages/lang_yygq"),
    --Add new language file to languages folder. Attention, new language won't show in-game when you add language
}
local publicText = {
    block = {
        "Z", "S", "J", "L", "T", "O", "I",
...
local function langFallback(T0, T)
    for k, v in next, T0 do
        if type(v) == "table" and not v.refuseCopy then
            --refuseCopy: just copy pointer, not contents
            if not T[k] then
                T[k] = {}
            end
...
local tipMeta = { __call = function(L)
