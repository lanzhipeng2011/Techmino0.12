...
function WIDGET.newText(D)
    --name,x,y[,color][,font=30][,align="M"][,hide]
    local _ = {
        name = D.name,
        x = D.x,
        y = D.y,
...
function WIDGET.newButton(D)
    --name,x,y,w[,h][,color][,font],code[,hide]
    if not D.h then
        D.h = D.w
    end
    local _ = {
...
function WIDGET.newSlider(D)
    --name,x,y,w[,unit][,smooth][,font][,change],disp,code,hide
    local _ = {
        name = D.name,
...
function selector:arrowKey(isLeft)
    local s = self.select
    if isLeft and s == 1 or not isLeft and s == #self.list then
        return
    end
    if isLeft then
        s = s - 1
...
function WIDGET.newSelector(D)
    --name,x,y,w[,color],list,disp,code,hide
    local _ = {
        name = D.name,
...
function WIDGET.newTextBox(D)
    --name,x,y,w[,h][,font][,secret][,regex],hide
    local _ = {
        name = D.name,
...
function WIDGET.lnk_BACK()
    SCN.back()
end
function WIDGET.lnk_CUSval(k)
    return function()
        return CUSTOMENV[k]
    end
end
function WIDGET.lnk_CUSrev(k)
    return function()
        CUSTOMENV[k] = not CUSTOMENV[k]
    end
end
function WIDGET.lnk_CUSsto(k)
    return function(i)
        CUSTOMENV[k] = i
    end
end

function WIDGET.lnk_SETval(k)
    return function()
        return SETTING[k]
    end
end
function WIDGET.lnk_SETrev(k)
    return function()
        SETTING[k] = not SETTING[k]
    end
end
function WIDGET.lnk_SETsto(k)
    return function(i)
        SETTING[k] = i
    end
end

function WIDGET.lnk_STPval(k)
    return function()
        return sceneTemp[k]
    end
end
function WIDGET.lnk_STPrev(k)
    return function()
        sceneTemp[k] = not sceneTemp[k]
    end
end
function WIDGET.lnk_STPsto(k)
    return function(i)
        sceneTemp[k] = i
    end
end
function WIDGET.lnk_STPeq(k, v)
    return function()
        return sceneTemp[k] == v
    end
end

function WIDGET.lnk_pressKey(k)
    return function()
        love.keypressed(k)
    end
end
function WIDGET.lnk_goScene(t, s)
    return function()
        SCN.go(t, s)
    end
end
function WIDGET.lnk_swapScene(t, s)
    return function()
        SCN.swapTo(t, s)
    end
end
...
function WIDGET.keyPressed(key)
    ...
    elseif key == "up" or key == "down" or key == "left" or key == "right" then
        ...
        if swap_xy then
            WX, WY = WY, WX
        end -- note that we do not swap them back later
