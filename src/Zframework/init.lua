SCR = require("src/ZFramework/screen")
COLOR = require("src/ZFramework/color")
SCN = require("src/ZFramework/scene")
LOG = require("src/ZFramework/log")
require("src/ZFramework/toolfunc")

VIB = require("src/ZFramework/vibrate")
SFX = require("src/ZFramework/sfx")

LIGHT = require("src/ZFramework/light")
BG = require("src/ZFramework/background")
WIDGET = require("src/ZFramework/widget")
TEXT = require("src/ZFramework/text")
sysFX = require("src/ZFramework/sysFX")

IMG = require("src/ZFramework/image")
BGM = require("src/ZFramework/bgm")
VOC = require("src/ZFramework/voice")

LANG = require("src/ZFramework/languages")
TASK = require("src/ZFramework/task")
FILE = require("src/ZFramework/file")
PROFILE = require("src/ZFramework/profile")
...
function love.wheelmoved(x, y)
    ...
    if wheelMoved[SCN.cur] then
        wheelMoved[SCN.cur](x, y)
    end
end
...
function love.keypressed(i)
    ...
    if devMode then
        ...
        elseif i == "f4" then
            if not kb.isDown("lalt", "ralt") then
                LOG.copy()
            end
        elseif i == "f5" then
        ...
        elseif i == "f8" then
            devMode = nil
            LOG.print("DEBUG OFF", COLOR.yellow)
        elseif i == "f9" then
            devMode = 1
            LOG.print("DEBUG 1", COLOR.yellow)
        elseif i == "f10" then
            devMode = 2
            LOG.print("DEBUG 2", COLOR.yellow)
        elseif i == "f11" then
            devMode = 3
            LOG.print("DEBUG 3", COLOR.yellow)
        elseif i == "f12" then
            devMode = 4
            LOG.print("DEBUG 4", COLOR.yellow)
...
function love.gamepadpressed(_, i)
    ...
    if gamepadDown[SCN.cur] then
        gamepadDown[SCN.cur](i)
    elseif keyDown[SCN.cur] then
        keyDown[SCN.cur](keyMirror[i] or i)
    elseif i == "back" then
        SCN.back()
    else
        WIDGET.gamepadPressed(i)
    end
end
...
function love.errorhandler(msg)
    ...
    local errScrShot
    gc.captureScreenshot(function(_)
        errScrShot = gc.newImage(_)
    end)
    gc.present()
    ...
    return function()
        ...
        if needDraw then
            ...
            setFont(100)
            gc.print(":(", 100, 40, 0, 1.2)
            setFont(40)
            gc.printf(text.errorMsg, 100, 200, SCR.w0 - 100)
            setFont(20)
...
function love.run()
    ...
    return function()
        ...
        if not mini() then
            ...
            if FCT >= 100 then
                ...
                if devMode then
                    ...
                    ins(frameTimeList, 1, dt)
                    rem(frameTimeList, 126)
                    gc.setColor(1, 1, 1, .3)
        ...
        while Timer() - lastFrame < 1 / 60 - 5e-6 do
            WAIT(0)
        end
    end
end