...
--Load modules
require("src/Zframework")

require("src/parts/list")
require("src/parts/default_data")
require("src/parts/gametoolfunc")

SCR.setSize(1280, 720)--Initialize Screen size
FIELD[1] = newBoard()--Initialize field[1]

BLOCKS = require("src/parts/mino")
AIBUILDER = require("src/parts/AITemplate")
FREEROW = require("src/parts/freeRow")

TEXTURE = require("src/parts/texture")
SKIN = require("src/parts/skin")
PLY = require("src/parts/player")
AIFUNC = require("src/parts/ai")
MODES = require("src/parts/modes")
TICK = require("src/parts/tick")
...
SHADER = {}
for _, v in next, love.filesystem.getDirectoryItems("src/parts/shaders") do
    if love.filesystem.getRealDirectory("src/parts/shaders/" .. v) ~= SAVEDIR then
        local name = v:sub(1, -6)
        SHADER[name] = love.graphics.newShader("src/parts/shaders/" .. name .. ".glsl")
    else
        LOG.print("Dangerous file : %SAVE%/src/parts/shaders/" .. v)
    end
end

--Load background files from SOURCE ONLY
for _, v in next, love.filesystem.getDirectoryItems("src/parts/backgrounds") do
    if love.filesystem.getRealDirectory("src/parts/backgrounds/" .. v) ~= SAVEDIR then
        local name = v:sub(1, -5)
        BG.add(name, require("src/parts/backgrounds/" .. name))
    else
        LOG.print("Dangerous file : %SAVE%/src/parts/backgrounds/" .. v)
    end
end

--Load scene files from SOURCE ONLY
for _, v in next, fs.getDirectoryItems("src/parts/scenes") do
    if fs.getRealDirectory("src/parts/scenes/" .. v) ~= SAVEDIR then
        require("src/parts/scenes/" .. v:sub(1, -5))
    else
        LOG.print("Dangerous file : %SAVE%/src/parts/scenes/" .. v)
    end
end
...
if fs.getInfo("unlock.dat") then
    FILE.loadUnlock()
end
if fs.getInfo("data.dat") then
    FILE.loadData()
end
if fs.getInfo("key.dat") then
    FILE.loadKeyMap()
end
if fs.getInfo("virtualkey.dat") then
    FILE.loadVK()
end

if fs.getInfo("tech_ultimate.dat") then
    fs.remove("tech_ultimate.dat")
end
if fs.getInfo("tech_ultimate+.dat") then
    fs.remove("tech_ultimate+.dat")
end
